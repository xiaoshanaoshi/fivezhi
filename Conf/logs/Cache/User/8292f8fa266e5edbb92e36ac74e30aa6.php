...
<link  rel="stylesheet" href="<?php echo RES;?>/datetimepicker/css/bootstrap-combined.min.css">
<link rel="stylesheet" type="text/css" media="screen"  href="<?php echo RES;?>/datetimepicker/css/bootstrap-datetimepicker.min.css">
<script type="text/javascript"  src="<?php echo RES;?>/datetimepicker/js/jquery.min.js"></script> 
<script type="text/javascript"  src="<?php echo RES;?>/datetimepicker/js/bootstrap.min.js"></script>
<script type="text/javascript"  src="<?php echo RES;?>/datetimepicker/js/bootstrap-datetimepicker.min.js"  charset="utf-8"></script>
...
<div class="content" <?php if(session('isQcloud') == true): ?>style="float:center;"<?php endif; ?>>
  ...
  <a href='<?php echo U("Img/add");?>' title='新增图文自定义回复' class='btnGrayS vm bigbtn'><img src="<?php echo RES;?>/images/pic.png" class="vm" />新增自定义图文</a>　   -->　

	  <a href='<?php echo U("Img/xiumi");?> ' title='新增图文自定义回复' class='btnGrayS vm bigbtn'><img src="<?php echo RES;?>/images/pic.png" class="vm" />新增图文</a>　
	...
	<button class="btnGrayS" style="height: 29px;">搜索</button>
          ...
          <div class="msgWrap">
            <TABLE class="ListProduct" border="0" cellSpacing="0" cellPadding="0" width="100%">
              ...
              <TBODY>
                <TR></TR>
				<form action="<?php echo U("Img/SavePublicDate");?>" method="post" id="sortform">
<?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "没有找到数据" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			    ...
			    <div id="datetimepicker_<?php echo $i;?>" class="input-append date">
			      <input type="text" name="public_time_<?php echo ($vo["id"]); ?>" id="time" value=<?php echo date('Y-m-d',$vo['publictime']) ?>></input>
			      <span class="add-on">
 ...
 <!--                 <td>
<input type="number" style="border:1px solid #c9c9c9;-webkit-border-radius: 3px;-moz-border-radius: 3px;border-radius: 3px;" 
class="usort" name="usort<?php echo ($vo["id"]); ?>" value="<?php echo ($vo["usort"]); ?>" />
                  ...
                  <td><?php echo date('Y-m-d',$vo['uptatetime']); ?></td>
				   ...
				   <!-- http://xiumi.us/studio/paper#/for/<?php echo $vo['id'];?> a target="ddd" href="<?php echo U('Wap/Index/content',array('token'=>$_SESSION['token'],'id'=>$vo['id']));?>">查看</a--> 
				   <a href="<?php echo U('Img/xiumi',array('id'=>$vo['id']));?>" title="编辑图文自定义回复">编辑</a>
            ...
            <button class="btnGrayS" onclick="$('#sortform').submit()" >保存</button>
<style>
	.usort {
		width:45px;
		height:23px;
